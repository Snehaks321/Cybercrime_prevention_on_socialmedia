...
def view_friend_list():
    ...
    data=select(q1)
    return render_template("user/view_friend_list.html",data=data)


@user.route("/sent_comment_for_the_post", methods=['GET', 'POST'])
def sent_comment_for_the_post():
    data={}
    # frnd_post_id = session['frnd_post_id']
    post_id = request.args.get('id')
    user_id=session['user_id']
    q="select * from comment inner join user on comment.user_id=user.user_id where post_id='%s'"%(post_id)
    data=select(q)
    if "submit" in request.form:
        comment = request.form['comment']
        d=predict_bully(comment)
        if d == "normal":
            q2 = "insert into comment values(null,'%s','%s','%s',curdate() , 'pending' , 'pending','Not' )" % (post_id, user_id  , comment)
            insert(q2)
        else:
            q2 = "insert into comment values(null,'%s','%s','%s',curdate() , 'pending' , 'pending','Bully' )" % (post_id, user_id  , comment)
            insert(q2)
            q2="update `user` set `bullying_count`=`bullying_count`+1 where `user_id`='%s'"%(user_id)
            update(q2)

        return f"<script>alert('comment sent');window.location='/view_post_of_friends';</script>"
